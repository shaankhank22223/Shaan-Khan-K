...
module.exports = function (defaultFuncs, api, ctx) {
  ...
  function send(form, threadID, messageAndOTID, callback, isGroup) {
    if (utils.getType(threadID) === "Array") sendContent(form, threadID, false, messageAndOTID, callback);
      ...
      var THREADFIX = String(threadID);
      
      if (global.Fca.isUser.includes(threadID)) {
        sendContent(form, threadID, true, messageAndOTID, callback);
      }
      else if (global.Fca.isThread.includes(threadID)) {
        sendContent(form, threadID, false, messageAndOTID, callback);
      }
      else if (isGroup !== null && isGroup !== undefined) {
        if (isGroup === true) {
          sendContent(form, threadID, false, messageAndOTID, callback);
        ...
        if (global.Fca.Data.event && global.Fca.Data.event.isGroup) {
          sendContent(form, threadID, false, messageAndOTID, callback);
          if (!global.Fca.isThread.includes(threadID)) global.Fca.isThread.push(threadID);
        } 
        else {
